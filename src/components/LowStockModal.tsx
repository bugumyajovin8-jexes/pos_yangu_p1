      ...
      <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
          <div className="flex items-center space-x-3 text-rose-600">
            ...
            <h2 className="text-xl font-bold text-slate-900">Bidhaa Zinazoisha (Low Stock)</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {lowStockProducts.length === 0 ? (
            <div className="text-center py-10">
              ...
              <p className="text-slate-500 font-medium">Hakuna bidhaa zinazoisha stock.</p>
            </div>
          ) : (
              ...
              {lowStockProducts.map((product: any) => (
                <div key={product.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-200">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center mt-1">
                        <span className="text-xs font-bold text-slate-500 mr-2">Stock:</span>
                        ...
                        <span className="text-xs text-slate-400 ml-2">(Min: {product.min_stock || 5})</span>
                      </div>
                    </div>
                  </div>
                  <button 
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">Ongeza Stock</span>
                  </button>
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
          <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl border border-slate-200">
            ...
            <p className="text-slate-500 mb-8">
              ...
              Stock ya sasa ni <span className="font-bold text-slate-900">{getValidStock(stockModalProduct)}</span>.
            </p>
