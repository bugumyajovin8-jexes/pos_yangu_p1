  ...
  const transaction = db.transaction(() => {
    ...
    if (products && products.length > 0) {
      ...
      for (const p of products) {
        stmt.run(p.id, shop_id, p.name, p.buy_price, p.sell_price, p.stock, p.low_stock_threshold, p.is_deleted ? 1 : 0, p.updatedAt || now);
      }
    }
    ...
    if (saleItems && saleItems.length > 0) {
      ...
      for (const item of saleItems) {
        stmtItem.run(item.id, item.saleId, item.productId, item.product_name, item.qty, item.buyPrice, item.sellPrice);
      }
    }
  });
